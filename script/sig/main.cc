...
int main(int argc, char** argv)
{
  ...
  RoSig roSigWrong;
  TTree treeWrong(Form("wrong_thr%d_wind%d", threshold, window) , Form("wrong_thr%d_wind%d", threshold, window));
  treeWrong.Branch("sigs", "sigs", &roSigWrong);
  treeWrong.SetAutoSave(0);
  treeWrong.SetAutoFlush(0);

  RoSig roSigPos;
  TTree treePos(Form("pos_thr%d_wind%d", threshold, window) , Form("pos_thr%d_wind%d", threshold, window));
  treePos.Branch("sigs", "sigs", &roSigPos);
  treePos.SetAutoSave(0);
  treePos.SetAutoFlush(0);
  
  RoSig roSigNeg;
  TTree treeNeg(Form("neg_thr%d_wind%d", threshold, window) , Form("neg_thr%d_wind%d", threshold, window));
  treeNeg.Branch("sigs", "sigs", &roSigNeg);
  treeNeg.SetAutoSave(0);
  treeNeg.SetAutoFlush(0);
    ...
    for(unsigned ev = 0; ev < nev; ev++) {
      ...
      for(auto &ch: chans) {
        ...
        for(int i = 0; i < peaks.size(); i++) {
          ...
          roSig.card = ch.card();
          roSig.level = level;
          if(max < 5000) roSig.unequal = 1;
          
          int p = peaks.at(i); roSig.peak = p;
          if(i < peaks_pos.size()) roSig.pol = 1; else roSig.pol = -1;
          
          if(p - pad1 > 0 && p + pad2 < cro.size()) {
            std::vector<int>::const_iterator first = cro.begin() + p - pad1;
            std::vector<int>::const_iterator last = cro.begin() + p + pad2;
            roSig.signal.insert(roSig.signal.end(), first, last);
            roSig.pedestal = peak.findPedestal(&cro, ped, window, fluctuations, p - pad1); 
            level = peak.findLevel(&roSig.pedestal); roSig.level = level;
            std::vector<int> additional_peaks = peak.findPeaks(&roSig.signal, roSig.level, roSig.pol, threshold);  
            roSig.peaks.insert(roSig.peaks.end(), additional_peaks.begin(), additional_peaks.end());
            int height = 0; int sum = 0; int sample = 0;
            for(int i = 0; i < roSig.signal.size(); i++) {
              sample = roSig.pol*(roSig.signal[i] - level);
              sum += sample;
            ...
            if(roSig.pol == -1) {
              roSigNeg = roSig;
              treeNeg.Fill(); roSigNeg.Clear(); roSig.Clear();
            }
          } 
          else {
            roSig.unequal = 1;
